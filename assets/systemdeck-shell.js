	...
	const SystemDeck = {
		...
		bindEvents: function () {
			...
			on(
				...
				function (e) {
					...
					if (!el.classList.contains("sd-closed")) {
						// CLOSING via Admin Bar
						if (self.currentDock === "min-dock") {
							// Exception: In min-dock → preserve state (don't reset)
							// sd_dock_state remains "min-dock", sd_last_dock has the pre-minimize dock
							...
							// Standard close → HARD RESET to default
							// Save current dock as lastDock for min-dock restore
							self.lastDock = self.currentDock
							localStorage.setItem("sd_last_dock", self.lastDock)
							// Reset dock state to standard-dock for next open
						...
						// OPENING via Admin Bar (soft open - don't switch docks)
						// init() already set the dock from sd_dock_state
			...
			on("#systemdeck #sd-close-button", "click", function () {
				// sd_dock_state is NOT modified - it stays as current dock
				// Next open will restore to the same dock position
				self.toggle()
