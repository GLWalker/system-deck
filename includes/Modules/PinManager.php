...
class PinManager
{
    ...
    public static function ajax_toggle_pin(): void
    {
        ...
        // Get current pins
        $pins = Locker::get_state((int)$user_id, $workspace_slug, 'pins') ?: [];
        ...
        if (isset($pins[$id])) {
            ...
            $layout = Locker::get_state((int)$user_id, $workspace_slug, 'layout') ?: [];
            if (is_array($layout)) {
                $layout = array_values(array_filter($layout, fn($item) => $item['id'] !== $id));
                Locker::save_state((int)$user_id, $workspace_slug, 'layout', $layout);
            }
        } else {
            $pins[$id] = $pin_data;
        }

        Locker::save_state((int)$user_id, $workspace_slug, 'pins', $pins);
