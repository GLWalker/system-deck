...
class Harvester
{
    ...
    {
        if (!class_exists('SystemDeck\Utils\Color')) return $settings;
        ...
        $process_palette = function(&$palette) {
            if (is_array($palette)) {
                foreach ($palette as &$p) {
                    if (isset($p['color'])) {
                        $c = new \SystemDeck\Utils\Color($p['color']);
        ...
        // 1. Standard Palettes
        if (isset($settings['color']['palette']['theme'])) $process_palette($settings['color']['palette']['theme']);
        if (isset($settings['color']['palette']['default'])) $process_palette($settings['color']['palette']['default']);
        if (isset($settings['color']['palette']['custom'])) $process_palette($settings['color']['palette']['custom']); // User overrides
    ...
    public static function needs_harvest(Context $context): bool
    {
        ...
        if (!$last || empty($last['settings'])) return true;
