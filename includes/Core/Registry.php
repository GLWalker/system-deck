...
class Registry {
    ...
    public function get_workspace(string $id): ?array {
        ...
        // Support Legacy storage (slug based on NAME) if ID storage empty?
        // Actually moving forward we should use ID. But for backward compat...
        // Let's check both.

        $saved_layout = Locker::get_state((int)$user_id, $real_slug, 'layout');
        if (!$saved_layout) {
            // Try legacy name-based slug
            $legacy_slug = sanitize_title($real_title);
            $saved_layout = Locker::get_state((int)$user_id, $legacy_slug, 'layout');
        }
    ...
    public function hydrate_manifest(string $id): array
    {
        ...
        // 2. Fetch User State
        $pins = Locker::get_state((int)$user_id, $slug, 'pins') ?: [];
        $layout = Locker::get_state((int)$user_id, $slug, 'layout');
